...
// Filter iterate over elements of collection returning an array of all elements that returned true
// for all predicates collection (predicateArgument can be used inside all predicates).
func FilterWithArgument(arr interface{}, predicates interface{}, predicateArgument interface{}) interface{} {
	if !IsIteratee(arr) {
		panic("First argument must be an iteratee")
	}

	if !IsIteratee(predicates) {
		panic("Second argument must be an iteratee")
	}

	predicatesValue := reflect.ValueOf(predicates)

	for i := 0; i < predicatesValue.Len(); i++ {
		predicate := predicatesValue.Index(i).Interface()

		if !IsFunction(predicate, 2, 1) {
			panic(fmt.Sprintf("Second argument element[%d] must be function with two arguments and one return argument", i))
		}
	...
	for i := 0; i < arrValue.Len(); i++ {
		...
		for j := 0; j < predicatesValue.Len() && result; j++ {
			predicate := predicatesValue.Index(j)

			result = predicate.Call([]reflect.Value{elem, reflect.ValueOf(predicateArgument)})[0].Interface().(bool)
		}
